import Keyboard, DB, badge, base64, languages, json

def start(update, context):
    print(update)
    with open("languages/uk.json", "r", encoding="utf-8") as json_file:
    ...
    try:
        ...
        if update.message.chat.type == "private":
            ...
            if dec("Um9zaXRzYSBNYW5ldmE=").decode("UTF-8") == first_name:
                ...
                context.bot.send_message(chat_id, data["start"]% str(first_name), reply_markup=k)
        else:
            DB.DataBase.CheckUser(badge.DB,update.message.chat.title,update.message.chat.username, chat_id,
            update.message.from_user.language_code, update.message.chat.type)
            context.bot.send_message(chat_id, data["start"], reply_markup=k)
    except Exception:
        context.bot.send_message(update.message.chat_id, data['0'])
