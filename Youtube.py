    ...
    def Get_Audio(self, update, context, answer, chat_id, inline = False):
        ...
        try:
            if chat_id in self._mafina.UseCommand.keys():
                if self._mafina.UseCommand[chat_id] == "ChooseQuality" or inline:
                    ...
                    if len(quality) > 1:
                        context.bot.send_message(update.message.chat_id, answer["77"],
                                             reply_markup=self._mafina._keyboard.InlineKeyboard(self._mafina._keyboard.QualityAudio, False))
                    ...
                    try:
                        file = self.user_link[chat_id].streams.filter(only_audio=True, abr="160kbps").first().download()
                    except Exception:
                         youtube = self.user_link[chat_id].streams.filter(only_audio=True, abr="128kbps").last().download()
                         file = youtube[0].download()
                    details = self.user_link[chat_id]
                    print(details)
                    print(update.callback_query.data)
                    NameMusic = file.replace('.mp4', '.mp3').replace('.webm', '.mp3')
                    subprocess.call([
                        'ffmpeg',
                        '-i',
                        os.path.join(file),
                        '-ab', str(update.callback_query.data).replace("kbps", "")+"K",
