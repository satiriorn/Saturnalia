        ...
        if badge.UseCommand[str(chat_id)] == "CutVideo":
            ...
            if durationSec>60:
                while (durationSec>=60):
                    durationSec -= 60
                    durationMin += 1
            command = ('ffmpeg -ss 00:{}:{} -i {} -to 00:{}:{} -c copy out.mp4').format(str(startMin), str(startSec),str(badge.CutFile[str(chat_id)]),str(durationMin), str(durationSec))
            os.system(command)
            context.bot.send_video(update.message.chat_id, open('out.mp4', 'rb'))
            remove('out.mp4')
