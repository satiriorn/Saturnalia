...
import Mafina, DB, os, pytube, Keyboard
from mutagen.easyid3 import EasyID3
from datetime import timedelta, datetime

def CutStart(update, context, answer, lang):
    context.bot.send_message(update.message.chat_id, answer["35"], reply_markup=Keyboard.InlineKeyboard(Mafina.Mafina.CutKeyboard[lang], False))

def CutAudio(update, context, answer, chat_id):
    NameMusic, ExtractName = "", ""
    try:
        if str(chat_id) in Mafina.Mafina.UseCommand.keys() and update.message.text == None:
            if Mafina.Mafina.UseCommand[str(chat_id)] == "CutAudio":
                file = context.bot.getFile(update.message.audio.file_id)
                title = update.message.audio.title
                if title!=None:
                    Mafina.Mafina.CutFile[str(chat_id)]=file.download(title)
                else:
                    Mafina.Mafina.CutFile[str(chat_id)] = "ExtractTrack.mp3"
                    file.download(title)
            context.bot.send_message(update.message.chat_id, answer["33"])
        elif str(chat_id) in Mafina.Mafina.UseCommand.keys()and update.message.text != None:
            if Mafina.Mafina.UseCommand[str(chat_id)] == "CutAudio":
                ...
                NameMusic = Mafina.Mafina.CutFile[str(chat_id)]
...
def CutVideo(update, context, answer, chat_id):
    try:
        if str(chat_id) in Mafina.Mafina.UseCommand.keys() and update.message.text == None:
            if Mafina.Mafina.UseCommand[str(chat_id)] == "CutVideo":
                file = context.bot.getFile(update.message.video.file_id)
                Mafina.Mafina.CutFile[str(chat_id)] = file.download()
                context.bot.send_message(update.message.chat_id, answer["33"])
        elif str(chat_id) in Mafina.Mafina.UseCommand.keys()and update.message.text != None:
            if Mafina.Mafina.UseCommand[str(chat_id)] == "CutVideo":
                Cut(update, context)
            ...
            Mafina.Mafina.UseCommand[str(chat_id)] = "CutVideo"
    except Exception:
        delete(update, chat_id)

def GetCutStart(update, context, answer, lang, chat_id):
    file, name = "", ""
    youtube = None
    try:
        if str(chat_id) in Mafina.Mafina.UseCommand.keys():
            if Mafina.Mafina.UseCommand[str(chat_id)] == "GetCutVideo":
                ...
                Mafina.Mafina.CutFile[str(chat_id)]= ('{}.mp4').format(name)
                context.bot.send_message(update.message.chat_id, answer["33"])
                Mafina.Mafina.UseCommand.pop(str(chat_id))
                Mafina.Mafina.UseCommand[str(chat_id)] = "CutEnd"
        else:
            context.bot.edit_message_text(chat_id=update.callback_query.message.chat_id, text=answer["1"],
                                          message_id=update.callback_query.message.message_id)
            Mafina.Mafina.UseCommand[str(chat_id)] = "GetCutVideo"
    except Exception:
        delete(update, chat_id)
        context.bot.send_message(chat_id, answer["2"])

def GetCutEnd(update, context, answer, chat_id):
    try:
        if str(chat_id) in Mafina.Mafina.UseCommand.keys():
            if Mafina.Mafina.UseCommand[str(chat_id)] == "CutEnd":
                Cut(update, context)
    except Exception:
        delete(update, chat_id)
        context.bot.send_message(chat_id, answer["2"])

def Cut(update, context, chat_id):
    time = str(update.message.text).split('-')
    start = GetDataTime(time[0])
    finish = GetDataTime(time[1])
    duration = finish - start
    start = datetime.strftime(start, '%H:%M:%S')
    file = str(Mafina.Mafina.CutFile[str(chat_id)])
...
def get_name(update):
    x =(lambda x: x == 'private' if update.message.chat.username else DB.DataBase.GetIdUser(Mafina.Mafina.DB, Mafina.Mafina.GetChatID(update)))(
...
def delete(update, chat_id):
    Mafina.Mafina.UseCommand.pop(str(chat_id))
    file = Mafina.Mafina.CutFile[str(chat_id)]
    Mafina.Mafina.CutFile.pop(str(chat_id))
