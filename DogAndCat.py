...
def SysAnimal(update,context):
    ...
    target_time = datetime.time(hour=9, minute=00, second=10).replace(tzinfo=target_tzinfo)
    print(DB.DataBase.GetCountAnimal(badge.DB, chat_id))
    for x in cursor:
...
def AnimalJob(context: telegram.ext.CallbackContext):
    ...
    context.bot.send_video(context.job.context, open("Animal/1.mp4", 'rb'))