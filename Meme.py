    ...
    def MoreMeme(self, update, context, lang, chat_id):
        ...
        if value[str(update.callback_query.data)] != "0":
            if chat_id in self._mafina.jobchat.keys():
                ...
                self._mafina.jobchat[chat_id] = self._mafina.job.run_repeating(self.MemeChatGroup, interval=int(value[str(update.callback_query.data)]), first=datetime.datetime.now(),
                                        context=chat_id)
            ...
            self._mafina.jobchat.pop(chat_id)
